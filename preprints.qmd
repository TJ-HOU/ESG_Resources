...
ssrn_fun<-function(url){
  ...
  title<-paste0("<a href=", html_attr(nodes, "href"),">",urltext,"</a>")
  # author
  #author<-raw%>%html_elements(".authors-list a")%>%html_text2()
  #author<-noquote(author)
  # date
  date<-raw%>%html_elements(".note-list span:nth-child(2)")%>%html_text2()
  date<- gsub("^.{0,8}", "", date)
  date<-noquote(date)
  # create ideas dataframe
...
ssrn<-ssrn_fun("https://papers.ssrn.com/sol3/JELJOUR_Results.cfm?form_name=journalBrowse&journal_id=235330&Network=yes&lim=false&orderBy=ab_approval_date&orderDir=desc&strSelectedOption=6")
library(DT)
